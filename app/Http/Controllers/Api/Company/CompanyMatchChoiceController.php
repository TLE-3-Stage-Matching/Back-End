...
class CompanyMatchChoiceController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $query = StudentMatchChoice::query()
            ->with(['student:id,email,first_name,last_name', 'vacancy:id,title,company_id'])
        ...
        $choices = $query->get();
        $data = $choices->map(fn (StudentMatchChoice $c) => $this->formatChoice($c))->all();

        return response()->json([
            'data' => $data,
            'links' => ['self' => url('/api/v2/company/match-choices')],
        ]);
    }

    /**
     * Approve a match choice. Only for vacancies belonging to the company.
     */
    public function approve(MatchChoiceDecisionRequest $request, StudentMatchChoice $choice): JsonResponse
    {
        $companyId = $request->user()->companyUser->company_id;
        $choice->load('vacancy');
        if (! $choice->vacancy || (int) $choice->vacancy->company_id !== (int) $companyId) {
            return response()->json(['message' => 'Match choice not found or not owned by your company.'], 404);
        }
        ...
        $choice->load(['student:id,email,first_name,last_name', 'vacancy']);

        return response()->json([
            'message' => 'Match choice approved.',
            'data' => $this->formatChoice($choice),
            'links' => [
                'self' => url("/api/v2/company/match-choices/{$choice->id}"),
                'collection' => url('/api/v2/company/match-choices'),
            ],
        ]);
    }

    /**
     * Reject a match choice. Only for vacancies belonging to the company.
     */
